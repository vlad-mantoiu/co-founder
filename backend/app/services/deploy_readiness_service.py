...
class DeployReadinessService:
    ...
    async def assess(self, clerk_user_id: str, project_id: str) -> dict:
        ...
        # For MVP: derive workspace files from sandbox_id + build_version metadata.
        # We attempt a minimal workspace snapshot from what we know about the build.
        # Real file content is not stored in DB — we synthesize representative check inputs.
        # Per plan spec: "For MVP: skip E2B fetch and instead check the Job's working_files
        # from Runner output". Since Job model doesn't store file content, we reconstruct
        # from build metadata (non-empty build = sandbox ran, so we use a representative
        # minimal workspace for checks that don't need actual file content).
...
def _reconstruct_workspace_for_checks(job: Job) -> dict[str, str]:
    ...
    For a completed build (READY status) with a build_version:
    - The sandbox ran successfully and executed health checks
    - We know it's a Python-based app (our runner generates FastAPI/Python code)
    - We include requirements.txt, README.md to represent a typical generated workspace
    ...
    """
    # Base workspace: Python backend (what our runner generates)
    workspace: dict[str, str] = {
        "requirements.txt": "fastapi>=0.100.0\nuvicorn>=0.23.0\nsqlalchemy>=2.0.0\n",
        "Procfile": "web: uvicorn app.main:app --host 0.0.0.0 --port $PORT",
    }

    # Add README.md if build has a workspace_path (indicates files were written)
    if job.workspace_path:
        workspace["README.md"] = f"# Project\n\nGenerated by AI Co-Founder.\n\nBuild version: {job.build_version or 'unknown'}\n"

    # Add .env.example based on preview_url presence (sandbox deployed)
    if job.preview_url:
        workspace[".env.example"] = "DATABASE_URL=postgresql://...\nSECRET_KEY=your_secret_here\nAPI_KEY=your_api_key_here\n"
