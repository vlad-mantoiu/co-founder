...
<task type="auto">
  <name>Task 1: Write tests for SSE events stream endpoint (RED)</name>
  <files>
    backend/tests/api/test_events_stream.py
  </files>
  <action>
    Create `backend/tests/api/test_events_stream.py` with unit tests for the new endpoint. Tests are written FIRST (TDD RED phase) — they will fail until Task 2 implements the endpoint.
  ...
  <verify>
    <automated>cd backend && python -m pytest tests/api/test_events_stream.py -x -v --tb=short 2>&1 | tail -5</automated>
    <manual>Test file created with 5 tests. Tests expected to FAIL (RED phase — endpoint not yet implemented).</manual>
  </verify>
  <done>
    - 5 tests written covering auth, 404, terminal-job-ready, terminal-job-failed, streaming response type
    - Tests use fakeredis (no real Redis required)
    - Tests use dependency overrides for auth and redis
    - Tests fail because endpoint does not exist yet (RED phase)
  </done>
</task>

<task type="auto">
  <name>Task 2: Add GET /{job_id}/events/stream typed SSE endpoint to jobs.py (GREEN)</name>
  <files>
    backend/app/api/routes/jobs.py
  </files>
  <action>
    Implement the SSE endpoint so that the tests from Task 1 pass (TDD GREEN phase).
  ...
  <verify>
    ...
    <manual>All 5 event stream tests pass (GREEN phase). Endpoint registered correctly. No import errors.</manual>
  </verify>
  <done>
    ...
    - All 5 tests from Task 1 now pass
  </done>
</task>
