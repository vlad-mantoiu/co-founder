...
must_haves:
  truths:
    - "Every narrated stage transition (SCAFFOLD, CODE, DEPS, CHECKS) in execute_build() fires a NarrationService.narrate() background task"
...
<task type="auto">
  ...
  <action>
    ...
    **Step 2: Wire NarrationService into generation_service.py**
    ...
    In `execute_build()`, after EACH state_machine.transition() call for the 4 narrated stages (SCAFFOLD, CODE, DEPS, CHECKS), add a narration fire-and-forget block. Pattern (repeats for each stage, changing the stage string):
    ```python
    ...
    - Do NOT narrate on STARTING (not in STAGE_AGENT_ROLES — STARTING is a setup step, not a build stage)
    ...
    **Step 4: Wire narration into execute_iteration_build()**
    ...
    Add the same narration pattern to execute_iteration_build() at every narrated stage transition (SCAFFOLD, CODE, DEPS, CHECKS — same 4 stages as execute_build()). Do NOT narrate STARTING or READY. Also move `_settings` resolution before STARTING transition in this method.
    ...
    **Step 5: Create test_narration_wiring.py**
    ...
    Tests:
    1. `test_narration_called_for_each_stage`: Mock _narration_service.narrate, run execute_build(), verify narrate() called 4 times (one per narrated stage: scaffold, code, deps, checks). Verify each call has correct stage string. Verify narrate() is NOT called for "starting" or "ready". Use `patch("app.services.generation_service._narration_service.narrate", new=generate_mock)`.
  ...
  <done>
    - NarrationService.narrate() called as fire-and-forget at 4 narrated stage transitions (SCAFFOLD, CODE, DEPS, CHECKS) in execute_build()
...
<success_criteria>
- NarrationService.narrate() called at 4 narrated stage transitions (SCAFFOLD, CODE, DEPS, CHECKS) in both execute_build() and execute_iteration_build()
