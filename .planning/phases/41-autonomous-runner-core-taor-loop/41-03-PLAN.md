...
must_haves:
  truths:
    ...
    - "Repeating the same tool call 3 times triggers a steering message ('try a different approach') and the loop continues; only a second RepetitionError terminates with repetition_detected"
...
1. **Setup:**
   ...
   - Create `IterationGuard(max_tool_calls=context.get("max_tool_calls", 150))` — note: IterationGuard must also have a `_had_repetition_warning = False` attribute (add it in __init__ if not already there, or set `guard._had_repetition_warning = False` after construction)
...
2. **TAOR Loop:**
   ...
   while True:
       ...
       tool_results = []
       steered = False  # Flag for first-strike repetition steering
       for tool_block in tool_use_blocks:
           try:
               ...
               # Two-strike repetition handling (per CONTEXT.md locked decision):
               # First strike: inject steering message, continue loop
               ...
               if not guard._had_repetition_warning:
                   ...
                   # Reset the repetition window so the guard doesn't immediately re-fire
                   guard._window.clear()
                   # Inject steering message as a user turn to redirect the model
                   # (assistant turn already appended above the for-loop)
                   messages.append({"role": "user", "content": [
                       {"type": "tool_result", "tool_use_id": tool_block.id, "content": "SYSTEM: You repeated the same tool call 3 times. Please try a completely different approach to achieve the same goal."},
                   ]})
                   steered = True
                   break  # Break inner tool dispatch loop, continue outer TAOR while-loop
               else:
       ...
       # Append tool results as user turn (skip if steered — steering already appended messages)
       if steered:
           continue  # Skip normal append, outer while-loop continues with steering message already in messages
...
**Two-strike repetition flow:**
The `IterationGuard` must have a `_had_repetition_warning: bool = False` attribute (add to `__init__` in `safety.py` if not already present). On first `RepetitionError`: set the flag to `True`, narrate "I noticed I'm repeating myself. Let me try a different approach.", clear `guard._window`, inject a steering tool_result message, set `steered = True`, and `break` out of the inner tool dispatch `for` loop. The `steered` flag causes the outer loop to `continue` without appending normal tool_results. On second `RepetitionError` (flag already `True`): terminate with `{"status": "repetition_detected"}`.
...
4. `test_loop_repetition_first_strike_continues` — Mock returns the same `bash` tool call with identical args 3 times (triggering first RepetitionError), then on the next iteration returns `stop_reason="end_turn"`. Assert `run_agent_loop()` returns `{"status": "completed", ...}` — the first repetition does NOT terminate, it steers and continues.

4b. `test_loop_repetition_second_strike_terminates` — Mock returns the same `bash` tool call with identical args 3 times (first strike → steering), then after the steering message, returns the same repeated tool call 3 more times (second strike). Assert `run_agent_loop()` returns `{"status": "repetition_detected", ...}` — only the second repetition terminates. Verify the messages list contains the steering content "try a completely different approach" (confirming the first-strike message was injected).
...
All tests are async (`pytest.mark.asyncio` or auto mode).
  ...
  <verify>
    ...
    <manual>All 11 TAOR loop tests pass with mocked Anthropic stream</manual>
  </verify>
  <done>11 TAOR loop tests pass — end_turn completion, tool dispatch, iteration cap, repetition first-strike continues, repetition second-strike terminates, truncation, system prompt verification, narration streaming, error handling, and protocol compliance all verified</done>
</task>

</tasks>

<verification>
- `cd backend && .venv/bin/pytest tests/agent/test_taor_loop.py -v` — all 11 pass
...
- 3x identical tool call triggers steering message and loop continues (first strike); second repetition returns status "repetition_detected" (second strike)
