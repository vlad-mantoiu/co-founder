...
must_haves:
  truths:
    ...
    - "Smart expand: ideas < 10 words trigger expand_needed flag, >= 10 proceed to questions"
  ...
  <action>
    ...
    **Methods:**
    ...
    1. `async def start_session(self, user_id: str, idea: str, tier_slug: str) -> OnboardingSession`:
       ...
       - Check smart expand: if `len(idea.split()) < 10`, set a flag `needs_expansion = True` on the returned session but still proceed (per Claude's discretion — word count check, not LLM call for MVP)
       - Actually per user decision: < 10 words prompts for more detail. Implementation: return session with `needs_expansion=True` in the response, but DO NOT block. The frontend decides whether to show the expand prompt. If they submit anyway, proceed.
