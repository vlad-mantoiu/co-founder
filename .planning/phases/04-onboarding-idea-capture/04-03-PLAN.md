...
<task type="auto">
  <name>Task 2: ConversationalQuestion + QuestionHistory + ProgressBar</name>
  <files>
    frontend/src/components/onboarding/ConversationalQuestion.tsx
    frontend/src/components/onboarding/QuestionHistory.tsx
    frontend/src/components/onboarding/ProgressBar.tsx
  ...
  <done>
    ConversationalQuestion renders one question at a time with mixed input types, skeleton shimmer, and keyboard navigation. QuestionHistory shows scrollable previous Q&A with seamless edit. ProgressBar animates fill as questions are answered. All components use Tailwind dark theme.
  </done>
</task>

<task type="auto">
  <name>Task 3: ThesisSnapshot with inline editing and tier-gated sections</name>
  <files>
    frontend/src/components/onboarding/ThesisSnapshot.tsx
  </files>
  <action>
    **ThesisSnapshot** (`frontend/src/components/onboarding/ThesisSnapshot.tsx`):
    ...
    Use Tailwind CSS for styling. Dark theme: bg-gray-950 background, white text, white/10 borders, white/5 card backgrounds. Use existing design patterns from the marketing site components.
  </action>
  <verify>
    - `cd frontend && npx tsc --noEmit` passes without type errors
    - ThesisSnapshot.tsx exists and exports component
    - `npm run build` succeeds (no build errors)
  </verify>
  <done>
    ThesisSnapshot shows hybrid card/document view with inline editing and tier-gated locked sections with upgrade prompts. Uses Tailwind dark theme.
  </done>
</task>

<task type="checkpoint:human-verify" gate="non-blocking">
  <name>Task 4: Visual verification of onboarding flow</name>
