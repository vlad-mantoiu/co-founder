...
must_haves:
  ...
  artifacts:
    - path: "backend/pyproject.toml"
      provides: "Updated ruff config with line-length=120, extend-ignore E501, and per-file-ignores for E402"
...
<task type="auto">
  ...
  <action>
    ...
    **Step 1: Update pyproject.toml ruff config**
    ...
    1. Change `line-length = 100` to `line-length = 120`
    2. Add `extend-ignore = ["E501"]` to `[tool.ruff.lint]` — ruff format already enforces line wrapping for wrappable code; E501 in lint is redundant and catches unfixable string/f-string literals that ruff format cannot split
    3. Add per-file-ignores for E402:
    ...
    The E501 suppression is correct because ruff format (which runs as a separate CI step) already enforces 120-char line wrapping for all reformattable code. The remaining E501 violations are in string literals and f-strings that no formatter can split.
    ...
    **Step 3: Run ruff format**
    ...
    This reformats 114 files — wrapping reformattable lines to 120 chars. ruff format respects the line-length setting in pyproject.toml. Note: string literals and f-strings longer than 120 chars are NOT reformattable — E501 is suppressed via extend-ignore for this reason.

    After these three steps, the only remaining errors should be the ~35 manual-fix items handled in Task 2 (F821, F841, E721, N806, UP042, N818).
  </action>
  <verify>
