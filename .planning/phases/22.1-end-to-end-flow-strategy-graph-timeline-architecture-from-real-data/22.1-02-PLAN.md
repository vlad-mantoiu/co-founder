...
<task type="auto">
  ...
  <action>
    **In `backend/app/api/routes/understanding.py`:**
    ...
    5. The pre-creation of artifact rows happens in the finalize_interview route AFTER finalize() returns, BEFORE background_tasks.add_task(). Create them with:
       ```python
       async with session_factory() as session:
           for at in [ArtifactType.STRATEGY_GRAPH, ArtifactType.MVP_TIMELINE, ArtifactType.APP_ARCHITECTURE]:
               artifact = Artifact(
                   project_id=UUID(result["artifact_id"]).  # Wait, need project_id
               )
       ```
       Actually, extract project_id from the finalize result. The finalize service returns artifact_id for the IDEA_BRIEF — we need to query the artifact to get project_id, or better: modify finalize() to also return project_id in the result dict.
