...
Note — CSP topology: The dashboard frontend is served via ECS/ALB (not CloudFront). CloudFront only serves the marketing site (getinsourced.ai). Therefore, the Next.js `headers()` function in `next.config.ts` is the ONLY CSP mechanism needed — ALB passes through Node.js HTTP response headers unmodified. No CDK/CloudFront changes required. The roadmap success criterion text mentioning "CDK headers" is slightly misleading for this app; it would only apply if CloudFront were in front of the dashboard, which it is not.
</objective>
...
- Confirm ALB-only topology: no CloudFront distribution serves the dashboard app, so Next.js `headers()` is the sole CSP mechanism. No CDK file changes needed.
  </verify>
  <done>
