...
2. **Render based on state:**
   ...
   **"expired" state:**
   - Full card replacing the iframe area entirely (per locked decision):
     ...
     - Per Claude's Discretion: screenshot capture mechanism is skipped this phase — no Playwright or headless browser in the backend to capture screenshots. A placeholder icon is used instead.
