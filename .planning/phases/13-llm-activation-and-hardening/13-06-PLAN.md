...
**IMPORTANT:** Add `get_redis` and `get_or_create_user_settings` as **module-level imports** (not inside the function body). This is required so that `unittest.mock.patch("app.domain.risks.get_redis", ...)` and `patch("app.domain.risks.get_or_create_user_settings", ...)` bind to the correct names in the module namespace. Patching local imports inside a function body does not work.
