...
must_haves:
  ...
  key_links:
    - from: "backend/app/services/generation_service.py"
      ...
      via: "JourneyService.transition_stage()"
      pattern: "transition_stage"
...
<task type="auto">
  ...
  <action>
    **Add post-build hook to GenerationService:**
    ...
    async def _handle_mvp_built_transition(
        ...
        async with factory() as session:
            ...
            # Transition to stage 3 (Development)
            await journey.transition_stage(
                project_id=uuid.UUID(project_id),
                new_stage=3,
                reason="MVP build completed",
                context={"build_version": build_version, "preview_url": preview_url}
            )
            # Log timeline event
            await journey.log_event(
                ...
                metadata={"preview_url": preview_url, "build_version": build_version}
            )
        ...
        try:
            ...
            await graph_service.sync_milestone_to_graph(
                project_id=project_id,
                milestone_id=f"mvp_built_{project_id}",
                title="MVP Built",
                status="done",
                metadata={"build_version": build_version, "preview_url": preview_url}
            )
        except Exception:
    ...
    **Write tests in `backend/tests/services/test_mvp_built_transition.py`:**
    ...
    1. `test_first_build_transitions_to_mvp_built` — Execute build with build_v0_1. Assert project.stage_number == 3 after completion. Assert StageEvent with event_type="mvp_built" exists in DB.
    ...
    3. `test_mvp_built_timeline_event_created` — After first build, query StageEvent table for event_type="mvp_built". Assert it exists with preview_url in metadata.
