...
files_modified:
  ...
  - backend/app/db/models/change_request.py
  ...
  - backend/alembic/versions/add_change_requests_table.py
autonomous: true
...
Output: Gate 2 options, change request model/service/routes, iteration tracking integration.
</objective>
...
<task type="auto">
  ...
  <action>
    **Create `backend/app/services/change_request_service.py`:**
    ...
    5. Compute alignment score: `compute_alignment_score(mvp_scope.current_content, existing_changes + [{"description": description}])`
    6. Get iteration count from IterationTracker (Redis)
    ...
    **Write tests in `backend/tests/services/test_gate2_and_change_requests.py`:**
    ...
    3. `test_create_change_request_creates_artifact` — Call ChangeRequestService.create_change_request(). Assert Artifact created with type="change_request" and correct content schema.
