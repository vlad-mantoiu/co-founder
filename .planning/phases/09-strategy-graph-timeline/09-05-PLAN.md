...
1. Update `handleCardClick` to be async and conditionally fetch node detail:
   ```tsx
   const handleCardClick = useCallback(async (item: TimelineItem) => {
     // Build base detail from timeline item fields
     ...
     // For decision items with a linked graph node, enrich with API detail
     if (item.type === "decision" && item.graph_node_id && projectId) {
       try {
         ...
         if (res.ok) {
           ...
           // Use graph node's created_at (more precise than timeline timestamp)
           base.created_at = detail.created_at ?? item.timestamp;
         }
       } catch {
         // Non-fatal: fall back to base detail without expandable sections
       }
     }

     setSelectedItem(null); // Clear any previous selection first
     // We need to store enriched detail separately since selectedItem is TimelineItem
     setEnrichedDetail(base);
   }, [projectId, getToken]);
   ```

2. Since `NodeDetailModal` currently receives `selectedItem ? timelineItemToNodeDetail(selectedItem) : null`, but we now need to store an enriched `NodeDetail`, add a new state variable:
   ```tsx
   const [enrichedDetail, setEnrichedDetail] = useState<NodeDetail | null>(null);
   ```

3. Update `handleCloseModal` to also clear `enrichedDetail`:
   ```tsx
...
4. Remove the `timelineItemToNodeDetail` function (no longer needed after this change).

5. Update the `NodeDetailModal` usage to use `enrichedDetail` directly instead of the adapter:
   ```tsx
   <NodeDetailModal
     node={enrichedDetail}
...
6. Update `handleViewInGraph` — it currently references `node.id` from the modal callback which passes the `NodeDetail.id`. Since `NodeDetail.id` for timeline items is `item.id` (timeline item ID, not node ID), but `onViewInGraph` navigates to `?highlight=${nodeId}`, we need to pass the `graph_node_id` not the timeline `id`. Fix this:
   - In `handleCardClick`, after enriching, if we have a `graph_node_id`, store it on enrichedDetail as `id` override, OR keep the `graph_node_id` separately.
   - Simpler: set `base.id` to `item.graph_node_id` when a graph node is linked, so `onViewInGraph` navigates to the correct graph node. Otherwise keep `item.id` (non-decision items don't have a graph link shown anyway since `showGraphLink={true}` but `onViewInGraph` is only called from the modal button).

   Actually the cleanest fix: in `handleCardClick`, set `base.id = item.graph_node_id ?? item.id`. The "View in Strategy Graph" button in NodeDetailModal calls `onViewInGraph(node.id)`, so this ensures it navigates to the correct graph node ID for linked decisions, and falls back to the timeline item ID for non-linked items (which won't show the View in Graph button since it requires `showGraphLink` and `onViewInGraph` but the button only appears when `showGraphLink && onViewInGraph` — it still appears since both are set, so for non-decision items with no graph_node_id just navigating with the timeline ID is a no-op in the strategy graph).

   Final `handleCardClick` outline:
   ```tsx
   const handleCardClick = useCallback(async (item: TimelineItem) => {
     const base: NodeDetail = {
...
7. `selectedItem` state can be removed since it's no longer used after this change (enrichedDetail replaces it for the modal).
