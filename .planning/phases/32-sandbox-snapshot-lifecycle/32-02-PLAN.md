...
1. In `backend/app/api/routes/generation.py`, add two new response schemas and endpoints:
   ...
   **POST /{job_id}/resume endpoint:**
   ...
   - On success:
     - Update Redis: `state_machine.set_field(job_id, "preview_url", new_preview_url)`
     - Update Redis: `state_machine.set_field(job_id, "sandbox_paused", "false")`
     - Update Redis: `state_machine.set_field(job_id, "updated_at", datetime.now(timezone.utc).isoformat())`  — resets expiry countdown
