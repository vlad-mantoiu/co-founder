...
2. In `backend/app/queue/worker.py`:
   ...
   - Place the auto-pause block AFTER the READY transition but BEFORE `_persist_job_to_postgres`. Then update `_persist_job_to_postgres` to accept `sandbox_paused`:
     a. Add `sandbox_paused: bool = False` parameter to `_persist_job_to_postgres` signature
     b. Inside the function, set `job.sandbox_paused = sandbox_paused` in the Job constructor kwargs
     c. At the call site, pass `sandbox_paused=True` when auto-pause succeeded (track with a local `paused_ok = True` flag after successful `beta_pause()` call)
