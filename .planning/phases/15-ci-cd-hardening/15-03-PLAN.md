...
<task type="auto">
  ...
  <action>
    ...
    **Rewrite `.github/workflows/test.yml`** to serve as the test gate for branch protection. The workflow runs on PRs targeting main AND on push to main. It contains two jobs: `test` (backend pytest + ruff) and `typecheck-frontend` (tsc --noEmit).
    ...
    - The `test` job runs `pytest -m "not integration"` (not `pytest -m unit`) to also catch unmarked tests during transition. Per user decision: "only pytest blocks deploys"
    - Ruff lint and format checks run INSIDE the `test` job (not separate job) — per user decision, ruff is part of the test gate, not a separate gate
