...
<task type="auto">
  ...
  <done>Every test file (except e2e and conftest) has a pytestmark for unit or integration. `pytest -m "not unit and not integration" --ignore=tests/e2e` collects 0 tests. Unit tests pass without external services.</done>
</task>
