...
<task type="auto">
  ...
  <action>
    ...
    - `GET /{project_id}` — returns full graph for a project
      ...
      - Verify project ownership via SELECT Project WHERE id=project_id AND clerk_user_id=user.user_id (use get_session_factory() from db.base)
      - 404 if not found
    ...
    - Use `from app.db.base import get_session_factory` for DB access (call `get_session_factory()` to obtain the factory — matching pattern in dashboard.py)
    ...
    - `GET /{project_id}` — returns timeline items with optional search/filter
      ...
      - 404 if not found
      - Create `TimelineService(get_session_factory())` and call `get_timeline_items(...)`
